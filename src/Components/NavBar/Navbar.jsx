...
function Navbar() {
    ...
    return (
        <div className='nav-div fixed w-full top-0 left-0 flex justify-around p-[16px] shadow-[0_1px_3px_-2px_black] bg-white'>
            ...
            <form onSubmit={handleSearch} className='flex items-center gap-[10px]'>
                <input
                    type="text"
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    placeholder="Search products..."
                    className='p-[8px] pr-[40px] border-[1px] border-gray-300 rounded-md focus:outline-none focus:border-red-600'
                />
                <button type="submit" className='bg-transparent border-none'>
                    <FontAwesomeIcon icon={faSearch} className='text-gray-400 hover:text-red-600 transition-colors duration-200' />
                </button>
            </form>
